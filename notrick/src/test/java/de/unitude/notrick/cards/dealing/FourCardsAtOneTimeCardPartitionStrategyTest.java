...
import de.unitude.notrick.cards.dealing.FourCardsAtOneTimeCardPartitionStrategy;
...
public class FourCardsAtOneTimeCardPartitionStrategyTest {
	...
	public void setUp() throws Exception {
		...
		sut = new FourCardsAtOneTimeCardPartitionStrategy(new GermanCardDeck(), hands);
	}
	...
	@Test
		public void testPart()
		{
			...
			Card firstCardOfLastPlayer = hands.get(hands.size()-1).getAllCards().get(0);
			assertEquals(GermanColor.GRUEN,firstCardOfLastPlayer.getColor());
			assertEquals(GermanType.UNTER,firstCardOfLastPlayer.getType());
			
			Card lastCardOfLastPlayer = hands.get(hands.size()-1).getAllCards().get(cardsPerPlayer-1);
