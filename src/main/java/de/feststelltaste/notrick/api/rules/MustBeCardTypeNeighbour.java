...
public class MustBeCardTypeNeighbour implements Rule {
    ...
    private CardSet getNeigbours(CardSet cardsOnTable, CardSet cardsToFilter) {
        ...
        if (cardsWithSameSuit.isEmpty()) {
            ...
            int lowestCardTypeOfRow = cardsOnTable.asList().get(cardsOnTable.size() - 1).getType()
                    .getPriority();

            neighbours = getImmediateNeighbours(cardsWithSameSuit, highestCardTypePriority,
                    lowestCardTypeOfRow);
    ...
    protected CardSet getImmediateNeighbours(CardSet cardsWithSameSuit, int highestPriority,
            int lowestPriority) {
